...
const DarkMode = () => {
    ...
    useEffect(() => {
        ...
        const switchTimeline = gsap.timeline({ paused: true });
        switchTimeline.to('.switch__icon:nth-child(1)', { duration: 0.3, opacity: isChecked ? 0 : 1 });
        switchTimeline.to('.switch__icon:nth-child(2)', { duration: 0.3, opacity: isChecked ? 1 : 0 }, 0);
        switchTimeline.play();
    ...
    return (
        <div style={{ scale: '1.4' }}>
            ...
            <label className="switch">
                <input
                    className={`switch__input ${isChecked ? 'active' : ''}`}
                    type="checkbox"
                    role="switch"
                    name="dark"
                    checked={isChecked}
